...
func TestSessionRegistry_SessionID_NotFound(t *testing.T) {
	...
	_, ok := sr.SessionID(domain.EntityID(999))
	if ok {
...
func TestSessionRegistry_AllSessionIDs(t *testing.T) {
	...
	sr.Register("session-1", domain.EntityID(1))
	sr.Register("session-2", domain.EntityID(2))
	sr.Register("session-3", domain.EntityID(3))
...
func TestSessionRegistry_ConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < numGoroutines; i++ {
		go func(i int) {
			defer wg.Done()
			sr.Register("session-"+string(rune('a'+i%26)), domain.EntityID(i))
		}(i)
...
func TestSessionRegistry_OverwriteRegistration(t *testing.T) {
	...
	sr.Register("session-1", domain.EntityID(100))
	sr.Register("session-1", domain.EntityID(200))
	...
	_, ok = sr.SessionID(domain.EntityID(100))
	if ok {
