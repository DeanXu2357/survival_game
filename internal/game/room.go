...
type Room struct {
	...
	subManager *pubsub.Manager[UpdateMessage]
...
func (r *Room) SendCommand(cmd protocol.Command) {
	select {
	case r.commands <- cmd:
	default:
...
func (r *Room) SendStaticData(clientIDs []string) {
	...
	envelope := protocol.ResponseEnvelope{
		EnvelopeType: protocol.StaticDataEnvelope,
...
func (r *Room) broadcastGameUpdate() {
	...
	envelope := protocol.ResponseEnvelope{
		EnvelopeType: protocol.GameUpdateEnvelope,
		Payload:      json.RawMessage(payloadBytes),
	}
