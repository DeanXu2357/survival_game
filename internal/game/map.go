...
type MapConfig struct {
	...
	Dimensions  Vector2D       `json:"dimensions" validate:"required"`
...
type SpawnPoint struct {
	ID       string   `json:"id" validate:"required,min=1"`
	Position Vector2D `json:"position" validate:"required"`
}

type WallConfig struct {
	ID       string   `json:"id" validate:"required,min=1"`
	Center   Vector2D `json:"center" validate:"required"`
	HalfSize Vector2D `json:"half_size" validate:"required"`
